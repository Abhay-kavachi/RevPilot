...
- **Signature Verification**: Validated successfully via strict HMAC-SHA256 (`200 OK`)
- **Webhook Arrival Evidence**:
  Received `payment_link.partially_paid` with `amount_paid = ₹200.00` (20000 paise). This independently updated the case `amount_recovered` to accurately reflect only the partial amount paid, preventing financial misalignment.
...
- **Payment 1 (Partial)**: `pay_TVglMvVj6rH8vg` (₹100.00) -> Webhook: `payment_link.partially_paid`
- **Payment 2 (Full Balance)**: `pay_TVh12fyv3oPy2t` (₹400.00) -> Webhook: `payment_link.paid`
- **Amount at Risk**: ₹500.00 (50000 paise)
- **Recovered Amount**: ₹500.00 (50000 paise)
- **Final Case Status**: `CaseStatus.RECOVERED`
- **Accounting Verification**: The first webhook correctly incremented `amount_recovered` by 10000 paise while preserving the `WAITING_FOR_OUTCOME` status. The second webhook incremented it by the remaining 40000 paise, struck the 50000 ceiling, and transitioned the case to `RECOVERED`. An explicit replay of the partial webhook resulted in zero overcounting.
